...
def test_model(model, data_test, epoch, save_folder_name='prediction'):
    """
        Test run
    """

    for batch, (images_t, img_name) in enumerate(data_test):
        print(img_name[0])
        stacked_img = torch.Tensor([]).cpu()
        for index in range(images_t.size()[1]):
            with torch.no_grad():
                image_t = Variable(images_t[:, index, :, :].unsqueeze(0).cpu())
                # print(image_v.shape, mask_v.shape)
                output_t = model(image_t)
                output_t = torch.argmax(output_t, dim=1).float()
                stacked_img = torch.cat((stacked_img, output_t))
        _ = save_prediction_image(stacked_img, img_name[0], epoch, save_folder_name)
...
def save_prediction_image(stacked_img, im_name, epoch, save_folder_name="result_images", save_im=True):
    ...
    # Save Image!
    export_name = os.path.basename(im_name)
...
def test_SEM(model, data_test,  folder_to_save):
    ...
    for i, (images) in enumerate(data_test):
        ...
        for j in range(images.size()[1]):
            image = Variable(images[:, j, :, :].unsqueeze(0).cpu())
